...
//	alert("DocumentType2:" + $("#docTypeID").val());
	...
	if ($("#docTypeID").val()) {
		...
		$.post("ajax_processing.php?action=submitLicense", { licenseID: $("#editLicenseID").val(),description: $("#licenseDescription").val(),shortName: $("#licenseShortName").val(), organizationID: $("#licenseOrganizationID").val(), organizationName: $("#organizationName").val(), consortiumID: $("#licenseConsortiumID").val(), documentTypeID: $("#docTypeID").val(), uploadDocument: fileName, archiveInd: archiveval, notetitle: $("#notetitle").val(), notebody: $("#notebody").val(), documentNoteTypeID: $("#documentNoteTypeID").val(),revisionDate: $("#revisionDate").val() } ,
